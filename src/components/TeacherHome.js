...
// to), and a menu item that opens the full analytics Dashboard the team
// already built — so everything links together instead of living in
...
const T = {
  en: {
    ...
    statMessages: "Messages Received", statFlagged: "Children Flagged", statMonth: "This Month",
    total: "total", newLabel: "new", cases: "cases",
    recentMessages: "Recent Messages", viewAll: "View all", noMessages: "No messages yet",
    noMessagesSub: "When a psychologist sends you a diagnosis report, it will appear here.",
    quickLinks: "Quick Links", openDashboard: "Open Full Analytics Dashboard",
    openDashboardSub: "See screening results, trends and flags across all children.",
    backToHome: "← My Home",
    search: "Search by child or school...", of: "of", messagesWord: "messages",
    view: "View", school: "School", score: "Score", sentOn: "Sent on", language: "Language",
    diagnosis: "Diagnosis Summary", domainScores: "Domain Scores",
    ...
    profileStaff: "Staff / Teacher Number", profileStatus: "Verification Status", verified: "Verified",
  ...
  af: {
    ...
    messagesSub: "Diagnoseverslae wat die sielkundige aan jou gestuur het.",
    profileSub: "Jou rekeningbesonderhede en verifikasiestatus.",
    statMessages: "Boodskappe Ontvang", statFlagged: "Kinders Gevlag", statMonth: "Hierdie Maand",
    total: "totaal", newLabel: "nuut", cases: "gevalle",
    recentMessages: "Onlangse Boodskappe", viewAll: "Sien almal", noMessages: "Nog geen boodskappe",
    noMessagesSub: "Wanneer 'n sielkundige 'n diagnoseverslag stuur, verskyn dit hier.",
    quickLinks: "Vinnige Skakels", openDashboard: "Maak Volledige Paneelbord Oop",
    openDashboardSub: "Sien siftingsresultate, neigings en vlae vir alle kinders.",
    backToHome: "← My Tuisblad",
    search: "Soek volgens kind of skool...", of: "van", messagesWord: "boodskappe",
    view: "Sien", school: "Skool", score: "Punt", sentOn: "Gestuur op", language: "Taal",
    diagnosis: "Diagnose Opsomming", domainScores: "Domeinpunte",
    ...
    profileStaff: "Personeel- / Onderwysernommer", profileStatus: "Verifikasiestatus", verified: "Geverifieer",
  ...
  xh: {
    ...
    messagesSub: "Iingxelo zohlolo ezithunyelwe nguSazi Sengqondo.",
    profileSub: "Iinkcukacha zeakhawunti yakho nemeko yokuqinisekiswa.",
    statMessages: "Imiyalezo Efunyenweyo", statFlagged: "Abantwana Abakhonjiweyo", statMonth: "Le Nyanga",
    total: "iyonke", newLabel: "entsha", cases: "amatyala",
    recentMessages: "Imiyalezo Yamva Nje", viewAll: "Jonga Yonke", noMessages: "Akukho miyalezo",
    noMessagesSub: "Xa isazi sengqondo sithumela ingxelo, izakuvela apha.",
    quickLinks: "Amakhonkco Akhawulezayo", openDashboard: "Vula Ideshibhodi Epheleleyo",
    openDashboardSub: "Jonga iziphumo zohlolo, iinkqubela nezikhombisi kubo bonke abantwana.",
    backToHome: "← Ikhaya Lam",
    search: "Khangela ngomntwana okanye isikolo...", of: "kwi", messagesWord: "imiyalezo",
    view: "Jonga", school: "Isikolo", score: "Amanqaku", sentOn: "Ithunyelwe ngo", language: "Ulwimi",
    diagnosis: "Isishwankathelo Sohlolo", domainScores: "Amanqaku eMihlaba",
...
const domainColors = { cognitive: "#009B8D", motor: "#6B2F8A", social: "#E8175D", emotion: "#F26522" };
...
export default function TeacherHome({ user, profile }) {
  ...
  // Full analytics Dashboard is a complete screen with its own sidebar/topbar —
  // render it standalone with a floating way back to this home screen.
  ...
  return (
    <div className="dashboard-layout rh-shell" style={{ "--rh-accent": "#F26522", "--rh-accent-soft": "#FFEFE3" }}>
      <RoleSidebar
        navItems={navItems}
        ...
        {activePage === "home" && (
          <>
            <RoleHero
              ...
              subtitle={t.homeSub}
            >
              <StatRing value={messages.length} max={10} color="#fff" label={t.statMessages} />
              <StatRing value={flaggedCount} max={10} color="#fff" label={t.statFlagged} />
              <StatRing value={thisMonthCount} max={10} color="#fff" label={t.statMonth} />
            </RoleHero>
            ...
            <div className="rh-home-grid">
              <div>
                <TodayList
                  ...
                  items={loading ? null : messages.slice(0, 5).map((m) => ({
                    icon: "✉", color: "#F26522",
                    title: m.childName, meta: `${m.school} · ${t.sentOn} ${formatDate(m.sentAt)}`,
                  }))}
                />
                <div className="rh-card">
                  ...
                  <button className="th-quicklink" onClick={() => setActivePage("messages")} style={{ marginBottom: 0 }}>
                    <div className="th-quicklink-icon">✉</div>
                    <div>
                      <div className="th-quicklink-title">{t.navMessages}</div>
                      <div className="th-quicklink-sub">{t.messagesSub}</div>
                    </div>
                    <div className="th-quicklink-arrow">→</div>
                  </button>
                </div>
              ...
              <div>
                <div className="rh-profile-card">
                  <div className="rh-profile-avatar" style={{ background: "#F26522" }}>
                    {displayName.charAt(0).toUpperCase()}
                  </div>
                  <div className="rh-profile-name">{displayName}</div>
                  <span className="rh-profile-role" style={{ background: "#F265221a", color: "#F26522" }}>{t.roleLabel}</span>
                  <div className="rh-profile-verified">✓ Verified account</div>
                  <div className="rh-chip-grid">
                    <div className="rh-chip">
                      <div className="rh-chip-value">{messages.length}</div>
                      <div className="rh-chip-label">{t.statMessages}</div>
                    </div>
                    <div className="rh-chip">
                      <div className="rh-chip-value">{flaggedCount}</div>
          ...
          <>
            <div className="search-bar">
              ...
              <span style={{ fontSize: 12, color: "var(--ink-faint)", fontWeight: 600, marginLeft: "auto" }}>
                {filtered.length} {t.of} {messages.length} {t.messagesWord}
              </span>
            </div>
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              {filtered.length === 0 ? (
                <div className="empty-state">
                  ...
                  <div className="empty-state-sub">{t.noMessagesSub}</div>
                </div>
              ) : (
                <div style={{ overflowX: "auto" }}>
                  <table className="data-table">
                    <thead>
                      <tr>
                        <th>{t.recentMessages.replace("Recent ", "")}</th>
                        <th>{t.school}</th>
                        <th>{t.score}</th>
                        <th>{t.sentOn}</th>
                        <th></th>
                    ...
                    <tbody>
                      {filtered.map((m) => (
                        <tr key={m.id}>
                          <td style={{ fontWeight: 700 }}>{m.childName}</td>
                          <td>{m.school}</td>
                          <td><strong>{m.childScore}%</strong></td>
                          <td style={{ color: "var(--ink-faint)", fontSize: 12 }}>{formatDate(m.sentAt)}</td>
                          <td><button className="btn btn-teal btn-sm" onClick={() => setSelected(m)}>{t.view}</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {selected && (
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div className="modal" style={{ maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: "12px 16px", background: "var(--teal-lt)", borderRadius: 12, marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 800, fontSize: 14, color: "var(--teal)" }}>{selected.school}</div>
                <div style={{ fontSize: 12, color: "var(--ink-mid)" }}>{t.sentOn} {formatDate(selected.sentAt)}</div>
              </div>
              <span className="pill pill-pink">{selected.childScore}%</span>
            </div>
 
            {selected.domains && (
              <div style={{ marginBottom: 20 }}>
                {["cognitive", "motor", "social", "emotion"].map((k) => (
                  <div className="domain-bar" key={k}>
                    <div className="domain-bar-header">
                      <span>{t[k]}</span>
                      <span>{selected.domains[k] || 0}%</span>
                    </div>
                    <div className="domain-bar-track">
                      <div className="domain-bar-fill" style={{ width: `${selected.domains[k] || 0}%`, background: domainColors[k] }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="report-section">
              <div className="report-section-title">{t.diagnosis}</div>
              <div style={{ fontSize: 13, lineHeight: 1.7, color: "var(--ink-mid)", whiteSpace: "pre-wrap" }}>
                {selected.diagnosis}
              </div>
            </div>
